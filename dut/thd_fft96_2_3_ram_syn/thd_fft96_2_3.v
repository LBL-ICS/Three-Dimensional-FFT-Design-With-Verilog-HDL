...
parameter MEM_RD_CY=2;              //memory read cycles 

parameter OD_VLD_ST    = OD_FFT_CY                       ; //500
parameter OD_VLD_FN    = OD_FFT_CY+RDY_CY                ; //500+442368=442,868
parameter TD_MEM_RD_ST = OD_FFT_CY+RDY_CY                ; //500+442368=442,868
parameter TD_MEM_RD_FN = OD_FFT_CY+2*RDY_CY              ; //500+2*442368=885,236
parameter TD_RDY_ST    = OD_FFT_CY+RDY_CY+MEM_RD_CY      ; //500+442368+2=442,870
parameter TD_RDY_FN    = OD_FFT_CY+2*RDY_CY+MEM_RD_CY    ; //500+2*442368+2=885,238
parameter TD_VLD_ST    = 2*OD_FFT_CY+RDY_CY+MEM_RD_CY    ; //2*500+442368=443,368
parameter TD_VLD_FN    = 2*OD_FFT_CY+2*RDY_CY+MEM_RD_CY  ; //2*500+2*442368=885,736

parameter THD_MEM_RD_ST = 2*OD_FFT_CY+2*RDY_CY+MEM_RD_CY    ; //2*500+2*442368+2=885,738
parameter THD_MEM_RD_FN = 2*OD_FFT_CY+3*RDY_CY+MEM_RD_CY    ; //2*500+3*442368+2=1,328,106
parameter THD_RDY_ST    = 2*OD_FFT_CY+2*RDY_CY+2*MEM_RD_CY  ; //2*500+2*442368+4=885,740
parameter THD_RDY_FN    = 2*OD_FFT_CY+3*RDY_CY+2*MEM_RD_CY  ; //2*500+3*442368+4=1,328,108
parameter THD_VLD_ST    = 3*OD_FFT_CY+2*RDY_CY+2*MEM_RD_CY  ; //3*500+2*442368+4=886,240
parameter THD_VLD_FN    = 3*OD_FFT_CY+3*RDY_CY+2*MEM_RD_CY  ; //3*500+3*442368+4=1,328,608
parameter OUT_MEM_RD_ST = 3*OD_FFT_CY+3*RDY_CY+2*MEM_RD_CY  ; //3*500+3*442368+4=1,328,608
parameter OUT_MEM_RD_FN = 3*OD_FFT_CY+4*RDY_CY+2*MEM_RD_CY  ; //3*500+4*442368+4=1,770,931, for a 50MHz MOF, it takes 35.41944 ms. 
parameter OUT_VLD_ST    = 3*OD_FFT_CY+3*RDY_CY+3*MEM_RD_CY  ; //3*500+3*442368+6=1,328,610
parameter OUT_VLD_FN    = 3*OD_FFT_CY+4*RDY_CY+3*MEM_RD_CY  ; //3*500+4*442368+6=1,770,933, for a 50MHz MOF, it takes 35.41944 ms. 
parameter COUNTER_FN = 5*RDY_CY                ; //5*442368=2,214,340
...
wire [23:0] nxt_cnt0 = (cnt0==COUNTER_FN-1) ? 24'd0 : 
                                            cnt0_en ? (cnt0+24'd1) : cnt0;
wire [23:0] nxt_cnt1 = (cnt1==COUNTER_FN-1) ? 24'd0 :      
                                            cnt1_en ? (cnt1+24'd1) : cnt1;
wire [23:0] nxt_cnt2 = (cnt2==COUNTER_FN-1) ? 24'd0 :      
                                            cnt2_en ? (cnt2+24'd1) : cnt2;
wire [23:0] nxt_cnt3 = (cnt3==COUNTER_FN-1) ? 24'd0 :      
                                            cnt3_en ? (cnt3+24'd1) : cnt3;
wire [23:0] nxt_cnt4 = (cnt4==COUNTER_FN-1) ? 24'd0 :      
	                                    cnt4_en ? (cnt4+24'd1) : cnt4;
wire [23:0] nxt_cnt5 = (cnt5==COUNTER_FN-1) ? 24'd0 :      
...
wire b0_mem_rd = st0_td_mem_rd | st2_td_mem_rd | st4_td_mem_rd;
wire b1_mem_rd = st1_td_mem_rd | st3_td_mem_rd | st5_td_mem_rd;
...
wire b2_mem_rd = st0_thd_mem_rd | st2_thd_mem_rd | st4_thd_mem_rd;
wire b3_mem_rd = st1_thd_mem_rd | st3_thd_mem_rd | st5_thd_mem_rd;
...
wire b4_mem_rd = st0_out_mem_rd | st2_out_mem_rd | st4_out_mem_rd;
wire b5_mem_rd = st1_out_mem_rd | st3_out_mem_rd | st5_out_mem_rd;
